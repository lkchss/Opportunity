...
## 2. Get the search brief (optional)
...
Prints suggested `queries` + the normalized profile as JSON. (With a
`profile.json`/`context.txt` present, just `python -m finder.cli --brief`.) Use
them as a starting point; refine or add your own.
